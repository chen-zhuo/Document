...
优点：图片文件存在且路径正确，在任何地方都能加载图片（除非删除图片）。
...
![QQ截图20210707000422](image/QQ截图20210707000422.png)

### 全角半角

**全角和半角是针对中文输入法输入字符的不同状态。**

全角模式：输入一个字符占用2个字符，汉字占用2个字符；

半角模式：输入一个字符占用1个字符，汉字占用2个字符；

使用中文输入法的用户有时会遇到这样的情况，输入中文某些字符或字母的时候，输出的形状比较奇怪。例如：间距正常的样子abcdefg，间距异常的样子ａｂｃｄｅｆｇ，这就是因为开启了全角模式。

有以下三种方法来切换全角/半角（**切换时必须要在中文输入法下执行**）：

方法一：在中文输入法中同时按下Shift+空格键。

方法二：点击输入法图标切换。

![4e168d5653bbf82081f957b8ba21056104a36e5a](image/4e168d5653bbf82081f957b8ba21056104a36e5a.jpg)

方法三：右击输入法，切换全角和半角。

![20210512212117536](image/20210512212117536.png)